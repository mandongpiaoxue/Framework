...
****發佈日期：2018.08.01
****思路：(仿Vue生命週期)
	第一步取得data內和methods內數據，便於操作
	第二步執行created內代碼，此時無法獲取任何元素
	第三步獲取HTML內的元素DOM，即el所指對象
	第四步對el內的所有元素進行操作
	第五步執行mounted內代碼
	第六步銷毀所有內創信息
****使用小說明：
	IE8模式下,會讀取注釋掉的元素，若需兼容IE8，內部請不要注釋，應直接刪除，不兼容IE8，可注釋
...
function Mic(obj) {
	...
	***********************************/
	...
	if(obj.created){obj.created.call(this);obj.created()};
	...
	*****************************************/
	...
	/* 處理m-for */
	if(forCollection){
		for(var i=0;i<forCollection.length;i++){
			var forParam=forCollection[i].value.split(' in ');	/* 將m-for的值用 in 分開 */
			var param=[],forEls={},forOS={};
			var forEle=forCollection[i].ownerElement;/* 獲取m-for所在的DOM元素 */
			var forParent=forEle.parentNode;/* 獲取DOM元素的父元素，用於添加DOM元素 */
			var obj=forEle.micData&&forEle.micData[trim(forParam[1])]?forEle.micData[trim(forParam[1])]:this[trim(forParam[1])];/* 判斷第二參數是否在micData中，如果在賦值，不存在則使用data中的數據 */
			var count=0;/* 定義計數器 */
			...
			if(obj instanceof  Array){/* 如果是數組型 */
				for (var j=0;j<obj.length;j++){/* 遍歷數組 */
					...
					forCollection=forCollection.concat(forOS);/* 將 */
					onForOS=offspring(forEls);/* 獲取DOM元素的子孫集合 */
					onForOS.forEach(function(item,index){
						if(onForOS[index].parentNode.micData){/* 如果父元素存在micData屬性 */
							onForOS[index].micData=cloneObject(onForOS[index].parentNode.micData);/* 拷貝父元素的micData屬性 */
						}else{onForOS[index].micData={};}/* 否則設為空 */
						onForOS[index].micData[param[0]]=obj[j];/* 將值放到第一位置（必須） */
						if(param[1])onForOS[index].micData[param[1]]=j;/* 將順序值放到第二位置（非必須），只有設置才有 */
					})
				}
				forParent.removeChild(forEle);/* 移除被複製的元元素 */
			}else if(obj instanceof  Object){/* 如果是對象型 */
				for(var name in obj){
					forEls=forEle.cloneNode(true);
					removeAttribute(forEls,'m-for');/* 刪除m-for屬性，防止死循環 */
					forParent.insertBefore(forEls,forEle);/* 添加子元素 */
					forOS=collection(offspring(forEls),'m-for');/* 獲取子元素的for循環集合 */
					forCollection=forCollection.concat(forOS);/* 將新循環鏈接到for集合中 */
					onForOS=offspring(forEls);
					onForOS.forEach(function(item,index){
						if(onForOS[index].parentNode.micData){/* 如果父元素存在micData屬性 */
							onForOS[index].micData=cloneObject(onForOS[index].parentNode.micData);/* 拷貝父元素的micData屬性 */
						}else{onForOS[index].micData={};}/* 否則設為空 */
						onForOS[index].micData[param[0]]=obj[name];/* 將值放到第一個位置 */
						if(param[1])onForOS[index].micData[param[1]]=count;/* 將順序值放到第二位置（非必須），只有設置才有 */
						if(param[2])onForOS[index].micData[param[2]]=name;/* 將名稱值放到第三位置（非必須），只有設置才有 */
					})
	...
	var textCollection = collection(eleOffspring, 'm-text');/* 獲取text集合 */
	
	/* 再處理m-text */
	if (textCollection){
		var that=this;
		for(var i=0;i<textCollection.length;i++){
			var textEle=textCollection[i].ownerElement;/* 獲取m-text屬性的所在DOM元素 */
			var textEleInfo=textEle.micData?textEle.micData:{};/* 獲取元素的micData值，即m-for中遍歷出來的值 */
			var textEleValue=textCollection[i].value;/* 獲取m-text的值 */
			/* 解析數據 */
	...
	/* 解析{{}}數據 */
	for(var i=0;i<eleOffspring.length;i++){
		var that=this;
		var lc=[],rc=[];/* lc:left split collection;rc:right split collection */
		if((eleOffspring[i].innerHTML.search('{{')>=0)&&(eleOffspring[i].innerHTML.search('}}')>=0)){
			var ci=eleOffspring[i].micData?eleOffspring[i].micData:{};
			lc=eleOffspring[i].innerHTML.split('{{');
			for(var k=1;k<lc.length;k++){
				rc=lc[k].split('}}');
				rc[0]=strEval(trim(rc[0]),ci,that);
				try {
					lc[k]=eval(rc[0])+rc[1];
				} catch (error) {
					lc[k]='請檢查大括號內語句';
			...
			eleOffspring[i].innerHTML=lc.join('');
		}
	}
	...
	if(ifCollection){
		...
		for(var i=0;i<ifCollection.length;i++){
			var ifEle=ifCollection[i].ownerElement;
			var ifEleDate=ifEle.micData?ifEle.micData:{};
			/* 處理m-else */
			...
			var sc=strCompare(ifCollection[i].value);
			/* 壓縮掉兩側的空格和() */
			var scv1=trim(sc[0]);
			var scv2=trim(sc[1]);
			/* 判斷有值，防止() */
	...
	***********************************/
	...
	/* 將mounted的作用域設置在自己體內 */
	if(obj.mounted){obj.mounted.call(this);obj.mounted()};
	...
	*********************************/
	...
	/* 移除所有vue屬性 */
	var attributes=['m-text','m-if','@',':','m-else'];
	removeAttribute(eleOffspring,attributes);

	/* 移除所有元素的micData */
	eleOffspring.forEach(function(item,index) {
		try {/* IE8不支持delete,用此法兼容 */
			delete(eleOffspring[index].micData);
		} catch (error) {
			eleOffspring[index].micData={};/* 在IE8中設置為空即可清除 */
		}
    })
}
	...
	els.forEach(function(item,index){/* 遍歷DOM元素集 */
		for(var i=0;i<item.attributes.length;i++){/* 遍歷DOM元素集的屬性集 */
			...
			if(attr.length==1){/* 如果屬性是開頭標記 */
				if(attrName.substring(0,1)==attr)collection.push(item.attributes[i]);
			}else{
				if(attrName==attr)collection.push(item.attributes[i]);
			}
		}
	});
	return collection;
