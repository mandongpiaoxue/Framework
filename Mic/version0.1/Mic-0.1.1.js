...
****相比Vue的優點：
	...
	2、vue的v-model模式下不加括號不計算，本框架已支持
****升級說明：
	1、調整{{}}深層次不轉化的bug
	2、增加m-model功能支持
	3、優化數據模式
...
function Mic(obj) {
	...
	*****************************************/
	...
	for(var i=0;i<eleOffspring.length;i++){
		var that=this;
		var flag=false;
		if((eleOffspring[i].innerHTML==eleOffspring[i].innerText)&&(eleOffspring[i].innerText.search('{{')>=0)&&(eleOffspring[i].innerText.search('}}')>=0)){	
			flag=false;	
			var text="'"+eleOffspring[i].innerText.replace(/{{/g,'\'+').replace(/}}/g,'+\'')+"'";
			for(name in eleOffspring[i].attributes){
				if(eleOffspring[i].attributes[name].name=='m-text')flag=true;
			}
			if(!flag)eleOffspring[i].setAttribute('m-text',text);
		}
	}
	...
	if(ifCollection){
		...
		for(var i=0;i<ifAttCol.length;i++){
			...
			/* 處理m-else */
			var nextEle=ifEle.nextElementSibling;/* 獲取下一個兄弟元素 */
			var elseFlag=false;
	...
	if(modelCollection){
		...
		for(var i=0;i<modelAttCol.length;i++){
			...
			if (textCollection){
				for(var i=0;i<textAttCol.length;i++){
					...
					if (textEleValue.search(pattern)>=0){
						textEle.micActiveData={};
						textEle.micActiveData.data=cloneObject(textEleInfo);
			...
			modelEle.oninput=function(){
				...
				that[modelEleValue]=that[modelEleValue].replace(/\\/g,"\\\\").replace(/\'/g,"\\\'");
				if (textCollection){
					for(var i=0;i<textAttCol.length;i++){
						var textElement=textElsCol[i];/* 獲取m-text屬性的所在DOM元素 */
						if (textElement.micActiveData.text.search(pattern)>=0){
							var tevn=strEval(textElement.micActiveData.text,textElement.micActiveData.data,that);
							tevn=tevn.replace(/\'\'\+/g,"").replace(/\+\'\'/g,"");
							if(tevn=="\'\+\\\'\\\'\'")tevn="''";
							try {
								tevn=eval(tevn);
