	...
	*****************************************/
	...
	if(forCollection){
		for(var i=0;i<forAttCol.length;i++){
			...
			var obj=forEle.__mic__.data&&forEle.__mic__.data[trim(forParam[1])]?forEle.__mic__.data[trim(forParam[1])]:this[trim(forParam[1])];/* 判斷第二參數是否在__mic__.data中，如果在賦值，不存在則使用data中的數據 */
			var count=0;/* 定義計數器 */
			...
			if(obj instanceof  Array){/* 如果是數組型 */
				for (var j=0;j<obj.length;j++){/* 遍歷數組 */
					...
					onForOS.forEach(function(item,index){
						...
						}else{onForOS[index].__mic__.data={};}/* 否則設為空 */
						onForOS[index].__mic__.data[param[0]]=obj[j];/* 將值放到第一位置（必須） */
				...
				for(var name in obj){
					...
					onForOS.forEach(function(item,index){
						...
						onForOS[index].__mic__.data[param[0]]=obj[name];/* 將值放到第一個位置 */
	...
	if(modelCollection){
		...
		for(var i=0;i<modelAttCol.length;i++){
			...
			if (textCollection){
				for(var j=0;j<textAttCol.length;j++){
					...
					if (textEleValue.search(pattern)>=0){
						...
						if(!textEle.micActiveData.data)textEle.micActiveData.data=cloneObject(textEle.__mic__.data);
				...
				for(var k=0;k<braces.length;k++){
					if(braces[k].parent){
						if(braces[k].data.search(pattern)>=0){
							...
							if(!braces[k].parent.micActiveData.data)braces[k].parent.micActiveData.data=cloneObject(braces[k].parent.__mic__.data);
						}
					}		
				}
			}
			...
			if(!modelEle.micActiveData.data)modelEle.micActiveData.data=cloneObject(modelEle.__mic__.data);
			/* 激活oninput */
